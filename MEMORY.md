...
- Athena voice hotfix as of June 29, 2026: robotic provider-failure Telegram messages were not caused by `SOUL.md`; they were hardcoded in `/opt/hermes/gateway/run.py` and bypassed Athena's persona. Live gateway now has provider/auth/rate-limit/credit failure copy that states blocker, effect, and next move in Athena voice, plus a narrow Telegram greeting guard for exact low-context openers. `SOUL.md` was also tightened, but the reliable fix for tiny greetings is the gateway guard. Backup: `/opt/hermes/gateway/run.py.bak.athena-casual-guard-20260629074019`. Use `scripts/chowmes-athena-gateway-voice-guard --check` after Hermes updates or container rebuilds, and `--apply --restart-gateway` if the guard is missing.
